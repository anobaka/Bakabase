...
export function DrmKeyCell({
  ...
  if (work.drmKey === "") {
    return (
      <span className="text-xs text-default-400">{t("resourceSource.dlsite.drmKey.none")}</span>
    );
  }

  return (
    <div className="flex items-center gap-1">
      <span className="text-xs font-mono">{isRevealed ? work.drmKey : "******"}</span>
      <Tooltip
        content={
          isRevealed
      ...
      >
        <Button isIconOnly size="sm" variant="light" onPress={() => setIsRevealed((v) => !v)}>
          {isRevealed ? (
      ...
      <Tooltip content={t("resourceSource.dlsite.drmKey.copy")}>
        <Button isIconOnly size="sm" variant="light" onPress={() => handleCopy(work.drmKey!)}>
