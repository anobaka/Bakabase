...
import TypeMismatchTip from "@/pages/bulk-modification2/components/BulkModification/ProcessValue/components/TypeMismatchTip";
...
const Editor = (props: Props) => {
  ...
  const [
    propertyTypesForManuallySettingValue,
    setPropertyTypesForManuallySettingValue,
  ] = useState<PropertyTypeForManuallySettingValue[]>([]);
  const [value, setValue] = useState<BulkModificationProcessValue>(
    propsValue ?? {
      type:
        availableValueTypes?.[0] ??
  ...
  const changeValue = useCallback(
    (
      patches: Partial<BulkModificationProcessValue>,
      ...
      switch (nv.type) {
        case BulkModificationProcessorValueType.ManuallyInput: {
          ...
          if (!nv.property) {
            ...
            const property =
              (pv?.properties?.find(
                (p) => p.id == nv.propertyId && p.pool == nv.propertyPool,
    ...
    if (value.value) {
      if (!value.followPropertyChanges && follow) {
        ...
        if (value.followPropertyChanges && !follow) {
          value.value = deserializeStandardValue(
            (
              await BApi.property.getPropertyBizValue(
                property.pool,
                property.id,
                { dbValue: value.value },
              )
                ...
                {propertyTypesForManuallySettingValue.map((pt) => {
                  return (
                    <DropdownItem
                      key={pt.type}
                      className={
                        pt.isAvailable ? "" : "text-gray-400 cursor-not-allowed"
                      }
  ...
  const renderTypeMismatchTip = () => {
    ...
    switch (value.type) {
      case BulkModificationProcessorValueType.ManuallyInput:
        ...
        break;
      case BulkModificationProcessorValueType.Variable:
        selectedType = variables?.find((v) => v.key == value.value)?.property
          .type;
        break;
    }
  ...
  const valueTypes: BulkModificationProcessorValueType[] =
    availableValueTypes ??
    bulkModificationProcessorValueTypes.map((x) => x.value);
