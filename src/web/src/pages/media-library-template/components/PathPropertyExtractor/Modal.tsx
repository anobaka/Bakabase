...
export default ({ locators: propsLocators, onSubmit, onDestroyed }: Props) => {
  ...
  const [locators, setLocators] = useState<Partial<PathPropertyExtractor>[]>(
    propsLocators ?? [],
  );
        ...
        return (
          <>
            <Select
              ...
              selectedKeys={
                locator.layer == undefined
                  ? undefined
                  : [locator.layer.toString()]
              }
  ...
  return (
    ...
    >
      <div className={"flex flex-col gap-2 min-h-0 overflow-auto"}>
        <Accordion
          selectedKeys={locators.map((l, i) => i.toString())}
          variant="splitted"
        >
      ...
      <div className={"flex flex-col gap-1"}>
        ...
        <div>
          {t<string>(
            "All rules will be run independently, and the results will be merged",
          )}
        </div>
      </div>
