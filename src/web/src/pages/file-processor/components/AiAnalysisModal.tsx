...
const AiAnalysisModal = ({ directoryPath, filePaths, onDestroyed }: Props) => {
  ...
  const [structureResult, setStructureResult] =
    useState<BakabaseModulesAIServicesFileStructureAnalysisResult | null>(null);
  const [namingResult, setNamingResult] =
    useState<BakabaseModulesAIServicesNamingConventionAnalysisResult | null>(null);
  const [nameCorrections, setNameCorrections] =
    useState<BakabaseModulesAIServicesFileNameCorrectionResult | null>(null);
  const [similarityResult, setSimilarityResult] =
    useState<BakabaseModulesAIServicesPathSimilarityGroupResult | null>(null);
  const [directoryCorrections, setDirectoryCorrections] =
    useState<BakabaseModulesAIServicesDirectoryStructureCorrectionResult | null>(null);
  ...
  const handleApply = useCallback(
    (tabKey: string, operations: BakabaseModulesAIServicesFileOperation[]) => {
          ...
          try {
            ...
            if (!r.code && r.data) {
              ...
              if (failureCount > 0) {
                ...
                toast.success(t<string>("fileProcessor.ai.applySuccess", { count: successCount }));
              }
  ...
  const analyzeStructure = async () => {
    ...
    try {
      const r = await BApi.ai.aiAnalyzeFileStructure({
        directoryPath,
  ...
  const analyzeNaming = async () => {
    ...
    try {
      const r = await BApi.ai.aiAnalyzeNamingConvention({
        filePaths,
  ...
  const suggestDirectoryCorrections = async () => {
    ...
    try {
      const r = await BApi.ai.aiSuggestDirectoryCorrections({
        directoryPath,
        referencePaths: referencePathsArray,
      });
  ...
  const getResultOperations = (
    tabKey: string,
  ): BakabaseModulesAIServicesFileOperation[] | undefined => {
    switch (tabKey) {
      case "structure":
        return structureResult?.operations;
      case "naming":
        return namingResult?.operations;
      case "nameCorrections":
        return nameCorrections?.operations;
      case "similarity":
        return similarityResult?.operations;
      case "directoryCorrections":
        return directoryCorrections?.operations;
      default:
        return undefined;
    }
  };
  ...
  const hasResult = (tabKey: string): boolean => {
    switch (tabKey) {
      ...
      case "naming":
        return namingResult !== null;
      case "nameCorrections":
        return nameCorrections !== null;
      case "similarity":
        return similarityResult !== null;
      case "directoryCorrections":
        return directoryCorrections !== null;
      default:
        return false;
    }
  };
  ...
  const renderResultContent = (tabKey: string): React.ReactNode => {
    switch (tabKey) {
              ...
              <div className="flex gap-4">
                {structureResult.totalFiles != null && (
                  <Chip size="sm" variant="flat">
                    {t("fileProcessor.ai.totalFiles")}: {structureResult.totalFiles}
                  </Chip>
                )}
                {structureResult.totalDirectories != null && (
                  <Chip size="sm" variant="flat">
                    {t("fileProcessor.ai.totalDirectories")}: {structureResult.totalDirectories}
                  </Chip>
                )}
              </div>
          ...
          <Card className="border border-default-200 rounded-md">
            <CardBody className="flex flex-col gap-2">
              {namingResult.detectedPattern && (
                <div className="flex flex-col gap-1">
                  <span className="text-sm font-medium text-default-600">
                    {t("fileProcessor.ai.detectedPattern")}
                  </span>
              ...
              {renderStringList(
                namingResult.inconsistencies,
                t("fileProcessor.ai.inconsistencies"),
              )}
          ...
          <Card className="border border-default-200 rounded-md">
            <CardBody className="flex flex-col gap-2">
              {nameCorrections.appliedConvention && (
                <div className="flex flex-col gap-1">
                  <span className="text-sm font-medium text-default-600">
                    {t("fileProcessor.ai.appliedConvention")}
                  </span>
              ...
              nameCorrections.corrections.filter((c) => {
                ...
                <div className="flex flex-col gap-2">
                  <span className="text-sm font-medium text-default-600">
                    {t("fileProcessor.ai.corrections")}
                  </span>
                  <div className="flex flex-col gap-1">
                    {nameCorrections.corrections
                      .filter((c) => {
                  ...
                  {similarityResult.groups.map((group, idx) => (
                    <div key={idx} className="flex flex-col gap-1">
                      <div className="flex items-center gap-2">
                        ...
                        {group.reason && (
                          <span className="text-xs text-default-400">{group.reason}</span>
                        )}
                      </div>
      ...
      case "directoryCorrections": {
        ...
        if (directoryCorrections.rawText)
          return renderRawTextFallback(directoryCorrections.rawText);
  ...
  const renderViewResultsButton = (tabKey: string) => {
    ...
    return (
      <Button size="sm" variant="flat" onPress={() => setResultModalTabKey(tabKey)}>
      ...
      >
        <div className="flex flex-col gap-1 mb-2 px-1">
          ...
          {filePaths.length > 0 ? (
            ...
            <div className="text-xs text-warning-500">{t("fileProcessor.ai.noPathsSelected")}</div>
          )}
        </div>
        {filePaths.length > 0 && (
          <div className="flex flex-col gap-1 mb-2 px-1">
            ...
            {referenceExpanded && (
              <div className="max-h-[160px] overflow-y-auto rounded bg-default-50 p-2 flex flex-col gap-0.5">
                {filePaths.map((p, idx) => (
                  ...
                  >
                    <span className="text-xs text-default-500 truncate" title={p}>
                      {p}
                    </span>
                  </Checkbox>
                ))}
        ...
        <Tabs disableAnimation variant="underlined">
          {/* Structure Analysis */}
          <Tab key="structure" title={t("fileProcessor.ai.analyzeStructure")}>
            <div className="flex flex-col gap-3 p-2">
              <p className="text-sm text-default-400">
                {t("fileProcessor.ai.analyzeStructure.description")}
              </p>
              <div className="flex items-center gap-2">
                <AiFeatureButton
                  color="primary"
                ...
                >
                  {loadingStructure
                    ? t("fileProcessor.ai.analyzing")
                    : t("fileProcessor.ai.analyzeStructure")}
                </AiFeatureButton>
          ...
          <Tab key="naming" title={t("fileProcessor.ai.analyzeNaming")}>
            <div className="flex flex-col gap-3 p-2">
              <p className="text-sm text-default-400">
                {t("fileProcessor.ai.analyzeNaming.description")}
              </p>
              <div className="flex items-center gap-2">
                <AiFeatureButton
                  color="primary"
                ...
                >
                  {loadingNaming
                    ? t("fileProcessor.ai.analyzing")
                    : t("fileProcessor.ai.analyzeNaming")}
                </AiFeatureButton>
          ...
          <Tab key="nameCorrections" title={t("fileProcessor.ai.suggestNames")}>
            <div className="flex flex-col gap-3 p-2">
              <p className="text-sm text-default-400">
                {t("fileProcessor.ai.suggestNames.description")}
              </p>
              <Input
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  {loadingNameCorrections
                    ? t("fileProcessor.ai.analyzing")
                    : t("fileProcessor.ai.suggestNames")}
                </AiFeatureButton>
          ...
          <Tab key="similarity" title={t("fileProcessor.ai.groupSimilarity")}>
            <div className="flex flex-col gap-3 p-2">
              <p className="text-sm text-default-400">
                {t("fileProcessor.ai.groupSimilarity.description")}
              </p>
              <Textarea
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  {loadingSimilarity
                    ? t("fileProcessor.ai.analyzing")
                    : t("fileProcessor.ai.groupSimilarity")}
                </AiFeatureButton>
          ...
          <Tab key="directoryCorrections" title={t("fileProcessor.ai.suggestDirectoryCorrections")}>
            <div className="flex flex-col gap-3 p-2">
              <p className="text-sm text-default-400">
                {t("fileProcessor.ai.suggestDirectoryCorrections.description")}
              </p>
              <div className="flex items-center gap-2">
                <AiFeatureButton
                  color="primary"
                ...
                >
                  {loadingDirectoryCorrections
                    ? t("fileProcessor.ai.analyzing")
                    : t("fileProcessor.ai.suggestDirectoryCorrections")}
                </AiFeatureButton>
