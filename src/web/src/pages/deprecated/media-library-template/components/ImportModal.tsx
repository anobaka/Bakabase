...
import type { PropertyType } from "@/sdk/constants.ts";
...
import { PropertyPool } from "@/sdk/constants.ts";
    ...
    <Modal
      ...
      footer={{
        ...
        okProps: {
          isDisabled:
            !shareCode &&
            !validate(propertyConversionsMap, extensionGroupConversionsMap, configuration),
      ...
      onOk={async () => {
        if (!configuration) {
          const importConfigurationResp =
            await BApi.mediaLibraryTemplate.getMediaLibraryTemplateImportConfiguration(shareCode!);
            ...
            configuration.uniqueCustomProperties.length > 0 && (
              <div className={"flex flex-col gap-2"}>
                <div className={"flex items-center gap-1 text-lg font-bold"}>
                  ...
                  <PropertiesMatcher
                    properties={configuration.uniqueCustomProperties.map((p) => ({
                      type: p.type,
                      name: p.name,
                    }))}
                    onValueChanged={(ps) => {
                    ...
                    {configuration.uniqueCustomProperties.map((p, pIdx) => {
                      ...
                      const property =
                        conversion?.toPropertyPool && conversion?.toPropertyId
                          ? propertyMap?.[conversion.toPropertyPool]?.[conversion.toPropertyId]
                          : undefined;
                      ...
                      return (
                        <>
                          ...
                          <div className={"flex items-center gap-1"}>
                            <BriefProperty fields={["name", "type"]} property={p} />
                          </div>
                  ...
                  >
                    {configuration.uniqueExtensionGroups.map((eg, egIdx) => {
                      ...
                      const leg = conversion?.toExtensionGroupId
                        ? extensionGroups?.find((g) => g.id == conversion.toExtensionGroupId)
                        : undefined;
                      const isSet = !!leg;
