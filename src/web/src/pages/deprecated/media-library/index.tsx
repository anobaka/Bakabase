...
import type { MediaLibraryTemplatePage } from "@/pages/deprecated/media-library-template/models";
...
import type { BakabaseServiceModelsViewUnknownResourcesCountViewModel } from "@/sdk/Api";
...
import TemplateModal from "@/pages/deprecated/media-library-template/components/TemplateModal";
...
const MediaLibraryPage = () => {
  ...
  const mediaLibraryUnknownResourcesCounts =
    unknownResourceCounts?.unknownPathCountByMediaLibraryId ?? {};
                ...
                <TableCell>
                  <div className="flex items-center gap-1">
                    <Tooltip
                      content={
                        ml.resourceCount > 0
                          ? t<string>("{{count}} resource(s) loaded in this media library.", {
                        ...
                        onSelectionChange={(keys) => {
                          ...
                          if (key === "moveResourcesToOtherMediaLibrary") {
                            createPortal(MediaLibrarySelectorV2, {
                              confirmation: true,
                              onSelect: async (toId: number) => {
                                if (toId === ml.id) {
                                  toast.warning(
                                    t<string>("Cannot move resources to the same media library"),
                                  );
                                ...
                                try {
                                  ...
                                  const idsResp = await BApi.resource.searchAllResourceIds(
                                    searchForm as any,
                                  );
                                  ...
                                  if (ids.length > 0) {
                                    await BApi.resource.setResourceMediaLibraries({
                                      ids,
                                      mediaLibraryIds: [toId],
                                    });
                                  }

                                  // Ask whether to delete the source media library after setting
                                  createPortal(Modal, {
                                    defaultVisible: true,
                                    title: t<string>("MediaLibrary.Confirm"),
                                    children: t<string>(
                                      "Move resources completed. Delete the source media library?",
                                    ),
                                    onOk: async () => {
                ...
                onValueChange={async (v) => {
                  if (!v) {
                    ...
                    if (keepResourcesTask) {
                        ...
                        createPortal(Modal, {
                          ...
                          children: (
                            <div className={"flex flex-col gap-3"}>
                              <div>
                                {t<string>("The GenerateResourceMarker task is currently running.")}
                              </div>
                              <div>
                                {t<string>(
                      ...
                      // Stop the task
                      try {
                        await BApi.backgroundTask.stopBackgroundTask("GenerateResourceMarker", {
                          confirm: true,
                        });
                      } catch (e) {
                      ...
                      createPortal(Modal, {
                        ...
                        children: (
                          <div className={"flex flex-col gap-2"}>
                            <div>
                              {t<string>("Do you want to delete all .bakabase.json marker files?")}
                            </div>
                                  ...
                                  while ((idx = buffer.indexOf("\n")) >= 0) {
                                    ...
                                    try {
                                      ...
                                      if (obj.type === "progress" || obj.type === "complete") {
                                        ...
                                        toast.warning(
                                          t<string>(
                                            "Operation stopped. Some marker files may remain and need to be deleted manually.",
                                          ),
                                        );
                            ...
                            <>
                                  ...
                                  <div className={"flex items-center gap-3"}>
                                    ...
                                    <div className={"flex-1"}>
                                      <div className={"font-medium"}>
                                        {t<string>(
                                          "Processing {{processed}} of {{total}} resources",
                                          {
                              ...
                              <Modal
                                footer={
                                  <div className={"flex justify-end gap-2"}>
                                    ...
                                    <Button color="danger" onClick={handleConfirmStop}>
                              ...
                              >
                                <div className={"flex flex-col gap-4"}>
                                  ...
                                  <div>
                                    {t<string>(
                                      "If you stop this operation now, you will need to delete the remaining marker files manually.",
                                    )}
                                  </div>
                                  <div
                                    className={"bg-[var(--bakaui-overlap-background)] p-3 rounded"}
                                  >
                                    ...
                                    <div className={"text-sm space-y-1"}>
                                      <div>
                                        {t<string>("File name: {{filename}}", {
                                          filename: ".bakabase.json",
                                        })}
                                      </div>
                                      <div>{t<string>("Location: Inside folder resources")}</div>
                                      <div>{t<string>("Attribute: Hidden file")}</div>
                                    </div>
                                  </div>
                                  <div className={"text-sm text-gray-600"}>
                                    {t<string>(
                                      "You can search for and delete these files using your file manager or terminal.",
                                    )}
                                  </div>
                                </div>
                              </Modal>
