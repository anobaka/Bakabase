...
export default ({ id, onSyncCompleted }: Props) => {
  ...
  const components = elementTypes.map((et) => {
    switch (et) {
      case "progress":
        return (
          <CircularProgress showValueLabel size="sm" value={task.percentage} />
        );
      case "cancel":
        return (
          <Button
      ...
      case "start":
        return (
          <Tooltip
            content={t<string>(
              "Once synced, you can search for resources in the media library",
            )}
            placement="top"
          >
            <Button
        ...
        const items: { value: any; tip?: any; icon?: any }[] = [
          {
            value: dayjs
              .duration(
                Math.ceil(moment.duration(task.elapsed).asSeconds()) * 1000,
              )
              .format("HH:mm:ss"),
            tip: undefined,
