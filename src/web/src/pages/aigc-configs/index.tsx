...
const AigcConfigsPage = () => {
  ...
  const [importResult, setImportResult] =
    useState<BakabaseModulesAIModelsInputAigcGeneratorComfyUIImportResult | null>(null);
  ...
  const importableProvidersByKind = useMemo(() => {
    ...
    for (const k of IMPORTABLE_KINDS) {
      map.set(
        k,
        providers.filter((p) => p.kind === k && p.aigcEnabled),
      );
    }
  ...
  const handleSave = async () => {
    ...
    if (isComfyUI) {
      ...
      try {
        ...
        toast.danger(
          t<string>("aigc.configs.error.workflowInvalidJson", { msg: e?.message ?? "" }),
        );
    ...
    <div className="flex flex-col gap-4 p-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button startContent={<AiOutlineCloudUpload />} variant="flat" onPress={handleOpenImport}>
      ...
      <p className="text-sm text-default-500 leading-relaxed">{t<string>("aigc.configs.intro")}</p>
            ...
            <TableRow key={v.generator.id}>
              ...
              <TableCell>
                <Chip size="sm" variant="flat">
                  {tMediaType(v.generator.mediaType)}
                </Chip>
              </TableCell>
              <TableCell>
                <Chip size="sm" variant="flat">
                  {tResourceMode(v.generator.resourceMode)}
                </Chip>
              </TableCell>
              <TableCell>
      ...
      <Modal isOpen={isOpen} scrollBehavior="inside" size="3xl" onClose={onClose}>
        <ModalContent>
          <ModalHeader>
            {isEditMode ? t<string>("aigc.configs.edit") : t<string>("aigc.configs.add")}
          </ModalHeader>
          <ModalBody>
            {editing && (
              <div className="flex flex-col gap-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  />
                  <Select
                    dataSource={RESOURCE_MODES.map((k) => ({
                      label: tResourceMode(k),
                      value: String(k),
                    }))}
      ...
      {/* Import */}
      <Modal isOpen={isImportOpen} scrollBehavior="inside" size="2xl" onClose={onImportClose}>
        <ModalContent>
          <ModalHeader>
          ...
          <ModalBody>
            <div className="flex flex-col gap-3">
              ...
              <p className="text-sm text-default-500">
                {t<string>(
                  `aigc.configs.import.kind.${AiProviderKindLabel[importKind]}.description`,
                )}
              </p>
              <Select
                isRequired
                dataSource={providersForImportKind.map((p) => ({
                  label: p.name,
                  value: String(p.id),
                }))}
                label={t<string>(
                  `aigc.configs.import.kind.${AiProviderKindLabel[importKind]}.providerLabel`,
                )}
                ...
                {importPaths.length === 0 ? (
                  ...
                  <div className="flex flex-col gap-1 max-h-64 overflow-y-auto">
                    {importPaths.map((p) => (
                      ...
                      >
                        <span className="text-xs font-mono truncate flex-1" title={p}>
                          {p}
                        </span>
                        <Button
                          isIconOnly
              ...
              <div className="flex flex-col gap-3">
                <div className="flex gap-3 flex-wrap">
                  <Chip color="success" variant="flat">
                    {t<string>("aigc.configs.import.imported", {
                ...
                <Table removeWrapper aria-label="import-result">
                  ...
                  <TableBody emptyContent={t<string>("common.empty")}>
                    {(
                      importResult.items as BakabaseModulesAIModelsInputAigcGeneratorComfyUIImportItemResult[]
                    ).map((it, idx) => (
                      <TableRow key={idx}>
                        <TableCell>
                          <Chip color={importStatusColor(it.status)} size="sm" variant="flat">
                            {t<string>(
                        ...
                        <TableCell>
                          <span className="text-xs font-mono break-all" title={it.path}>
                            {it.path}
                          </span>
                        </TableCell>
                        <TableCell>
...
const importStatusColor = (
  ...
  switch (status) {
    ...
    case 2:
      return "warning"; // SkippedDuplicate
    case 3:
    case 4:
      return "warning"; // SkippedInvalidJson / SkippedNotComfyUIWorkflow
    case 5:
