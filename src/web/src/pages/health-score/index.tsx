...
import { apiFilterGroupToInternal } from "@/pages/health-score/types";
...
const ProfileCard = ({
  ...
  const filter = useMemo(() => apiFilterGroupToInternal(p.membershipFilter), [p.membershipFilter]);
  ...
  return (
    <div className="border border-default-200 rounded-lg p-3 flex flex-col gap-2">
      <div className="flex items-center justify-between">
        ...
        <Switch isSelected={p.enabled ?? false} size="sm" onValueChange={onToggleEnabled} />
      </div>
      ...
      <div className="flex flex-col gap-1">
        ...
        {filterIsEmpty ? (
          <div className="text-xs text-default-500">{t<string>("healthScore.empty.noFilter")}</div>
        ) : (
      ...
      <div className="flex flex-col gap-1">
        ...
        {rules.length === 0 ? (
          <div className="text-xs text-default-500">{t<string>("healthScore.empty.noRules")}</div>
        ) : (
