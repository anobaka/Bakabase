...
/** The coordinator kept the session; asking for the same cursor again can succeed. */
const pageRetryCodes = new Set(["QuerySessionInterrupted", "Busy"]);
/** The session, its authorization or its source library changed; loaded pages cannot be continued. */
...
function canRetryPage(error?: Error) {
  // A transport failure (offline, aborted connection) never reached the session.
  if (!(error instanceof FederationError)) return true;
  if (sessionLevelCodes.has(error.code)) return false;
  if (pageRetryCodes.has(error.code) || error.retryable) return true;

  // Codes synthesized from a bare HTTP status: only a server-side failure is transient.
  return error.code.startsWith("Http") && error.status >= 500;
}
...
function Library() {
  ...
  const [preferences] = useState(() => {
    try {
      ...
      return { scope: undefined, sources: [] as string[] };
    }
  });
  // Without a choice in the URL or a saved preference, a device that can already read another
  // one starts on all of them. Decided once, from the first status, so a peer appearing later
  // never switches the scope (and re-runs the search) under the user.
  const defaultScope = useRef<Scope>();

  if (status && !defaultScope.current)
    defaultScope.current = status.peers.some((peer) => peer.enabled && peer.outboundGrant)
      ? "all"
      : "local";
  const savedScope = params.get("scope") ?? preferences.scope ?? defaultScope.current;
  ...
  // Retry the failed page with the same cursor only when that can succeed; otherwise the
  // snapshot itself is gone or no longer trustworthy and only a new search helps.
  const retryPage =
    !!firstPage &&
    !!lastPage?.nextCursor &&
    !expired &&
    !state.cancelled &&
    canRetryPage(state.error);
    ...
    <div className="mx-auto flex max-w-[1600px] flex-col gap-5 p-4 sm:p-6">
      ...
      {status?.browsingEnabled === true && (
        <>
          ...
          <ErrorNotice
            error={state.error}
            onRetry={() => (retryPage ? void nextPage() : submit())}
          />
