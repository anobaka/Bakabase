...
export default function ResourceDetail({
  ...
  /**
   * Asset IDs are short-lived leases issued with the detail. When one has expired, re-read the
   * detail and find the same file again; an ambiguous or missing match is not guessed at.
   */
  const renewAsset = async (asset: FederatedAsset, signal: AbortSignal, current: number) => {
    const response = await federationResourceApi.detail(resourceRef, signal);
    const found = response.resources.find((resource) => sameResource(resource.ref, resourceRef));

    if (current !== generation.current) return undefined;
    if (!found) throw new FederationError("ResourceGone", "", 404);
    setDetail(found);
    const matches = found.assets.filter(
      (candidate) =>
        candidate.kind === asset.kind &&
        candidate.fileName === asset.fileName &&
        (candidate.sourceRootId ?? null) === (asset.sourceRootId ?? null) &&
        (candidate.relativePath ?? null) === (asset.relativePath ?? null),
    );

    return matches.length === 1 ? matches[0] : undefined;
  };

  const play = async (requested: FederatedAsset, mode: "preview" | "player") => {
    const current = generation.current;
    let asset = requested;
    ...
    try {
      ...
      try {
        session = await federationResourceApi.playback(
          resourceRef,
          asset.assetId,
          mode,
          controller.signal,
        ...
        if (!(cause instanceof FederationError && cause.code === "AssetExpired")) throw cause;
        const renewed = await renewAsset(asset, controller.signal, current);
