...
export function getDockedPosition(edge: DockedEdge, current?: Position): Position {
  ...
  switch (edge) {
    case "left":
      return { x: -hiddenPart, y: cy };
    case "right":
      return { x: w - DOCKED_VISIBLE, y: cy };
    case "top":
      return { x: cx, y: -hiddenPart };
    case "bottom":
      return { x: cx, y: h - DOCKED_VISIBLE };
  }
}
  ...
  try {
    ...
    if (saved) {
      ...
      if (parsed.edge) {
        ...
        const hint: Position | undefined =
          parsed.freeAxis != null
...
export function useBakaDraggable(
    ...
    (e: React.MouseEvent) => {
      ...
      const handleMouseMove = (me: MouseEvent) => {
        ...
        if (isDraggingRef.current) {
          const newX = Math.max(
            -BAKA_SIZE + DOCKED_VISIBLE,
            Math.min(dragStartRef.current.px + dx, window.innerWidth - DOCKED_VISIBLE),
          );
          const newY = Math.max(
            -BAKA_SIZE + DOCKED_VISIBLE,
            Math.min(dragStartRef.current.py + dy, window.innerHeight - DOCKED_VISIBLE),
          );
          ...
          if (edge) {
            ...
            try {
              localStorage.setItem(
                ...
                JSON.stringify({
                  edge,
                  freeAxis: edge === "left" || edge === "right" ? pos.y : pos.x,
                }),
              );
            } catch {
              /* */
            ...
            try {
              localStorage.setItem(POSITION_STORAGE_KEY, JSON.stringify({ position: pos }));
            } catch {
              /* */
            }
          }
  ...
  const snapToEdge = useCallback((edge: DockedEdge) => {
    ...
    setPosition(pos);
    const freeAxis =
      edge === "left" || edge === "right" ? positionRef.current.y : positionRef.current.x;

    try {
      localStorage.setItem(POSITION_STORAGE_KEY, JSON.stringify({ edge, freeAxis }));
    } catch {
      /* */
    }
  }, []);
  ...
  const undockFromEdge = useCallback((edge: DockedEdge) => {
    ...
    switch (edge) {
      case "left":
        setPosition({ x: margin, y: cur.y });
        break;
      case "right":
        setPosition({ x: window.innerWidth - BAKA_SIZE - margin, y: cur.y });
        break;
      case "top":
        setPosition({ x: cur.x, y: margin });
        break;
      case "bottom":
        setPosition({ x: cur.x, y: window.innerHeight - BAKA_SIZE - margin });
        break;
    }
  }, []);
