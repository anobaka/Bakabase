...
import type { ChatConversation, UiMessage } from "./types";
  ...
  try {
    ...
    if (toolName === "SearchResources") {
      return Array.isArray(data?.Items)
        ? data.Items.map((r: any) => r.Id).filter((id: any) => id != null)
        : [];
    }
  } catch {
    /* ignore */
...
const ToolResourceCards: React.FC<{ toolName: string; result: string }> = ({
  toolName,
  result,
}) => {
  ...
  return (
    <div className="baka-chat-resource-cards">
      {resources.map((r) => (
        <ResourceCard key={r.id} disableCoverClick={false} resource={r} />
      ))}
    </div>
  );
};
...
const ChatView: React.FC<Props> = ({ mode, onClose }) => {
  ...
  const { isConfigured: modelConfigured, isLoading: modelLoading } = useAiFeatureConfigured(
    AiFeature.Chat,
  );
  ...
  const [warningDismissed, setWarningDismissed] = useState(() => {
    try {
      return localStorage.getItem("baka-chat-warning-dismissed") === "1";
    } catch {
      return false;
    }
  });
              ...
              .map((r: any) => {
                ...
                if (r.toolCalls) {
                  try {
                    ...
                    const results: { name: string; result?: string }[] = r.toolResults
                      ? JSON.parse(r.toolResults)
                      : [];
          ...
          switch (evt.type) {
            ...
            case ChatStreamEventType.ToolCallResult: {
              ...
              const lastCall = [...calls]
                .reverse()
                .find((c: { name: string }) => c.name === evt.toolName);
  ...
  const handleDismissWarning = useCallback(() => {
    ...
    try {
      localStorage.setItem("baka-chat-warning-dismissed", "1");
    } catch {
      /* */
    }
  }, []);
      ...
      <div className="baka-chat-main">
        <div className="baka-chat-header">
          ...
          <div className="baka-chat-header-actions">
            <Popover
              trigger={
                <button className="baka-chat-icon-btn" title={t("bakaChat.config.tooltip")}>
                  <SettingOutlined />
                </button>
              }
              visible={configOpen}
            ...
            >
              <div
                className="baka-chat-config-panel"
                role="button"
                tabIndex={0}
            ...
            {!isPage && (
              <>
                <Tooltip content={t("bakaChat.openFullPage.tooltip")}>
                  <button className="baka-chat-icon-btn" onClick={handleNavigateToPage}>
                    <ExpandAltOutlined />
                  </button>
                </Tooltip>
        ...
        {!modelLoading && !modelConfigured ? (
          ...
          <>
            <div ref={listRef} className="baka-chat-messages">
              {messages.length === 0 && (
              ...
              {messages.map((msg) => (
                <div key={msg.id} className={`baka-chat-msg baka-chat-msg-${msg.role}`}>
                    ...
                    return (
                      <div key={i} className="baka-chat-tool-call">
                        ...
                        {tc.result && isResourceTool && !tc.isLoading && (
                          <ToolResourceCards result={tc.result} toolName={tc.name} />
                        )}
                        {tc.result && (
                  ...
                  {msg.error && <div className="baka-chat-error">{msg.error}</div>}
                </div>
              ))}
            </div>
