...
import type { BakabaseModulesAIServicesFileOperation } from "@/sdk/Api";

import React, { useCallback, useMemo } from "react";
import { useTranslation } from "react-i18next";

import { Checkbox, Chip } from "@/components/bakaui";
...
const FileOperationTree = ({
  operations,
  selectedIndices,
  onSelectionChange,
}: FileOperationTreeProps) => {
        ...
        >
          ...
          <div className="flex flex-col gap-0.5 min-w-0 flex-1">
            <div className="flex items-center gap-2 text-sm flex-wrap">
              ...
              >
                {t(
                  `fileProcessor.ai.opType.${FileOperationTypeLabel[operation.type as FileOperationType]}`,
                )}
              </Chip>
              <span className="text-default-400 truncate" title={operation.sourcePath}>
              ...
              {operation.type !== FileOperationType.CreateDirectory && (
                <>
                  ...
                  <span
                    ...
                    title={operation.destinationPath}
                  >
            ...
            {operation.reason && (
              <span className="text-xs text-default-400">{operation.reason}</span>
            )}
          </div>
        </div>
  ...
  return <div className="flex flex-col gap-1">{roots.map((node) => renderNode(node, 0))}</div>;
};
