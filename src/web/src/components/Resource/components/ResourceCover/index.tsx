...
const ResourceCover = React.forwardRef((props: Props, ref) => {
  ...
  const urls = useMemo(() => {
    ...
    const coverPaths = discoveryState.coverPaths?.length
      ? discoveryState.coverPaths
      : resource.coverPaths?.length
        ? resource.coverPaths
        : [resource.path];
  ...
  }, [discoveryState, appContext.apiEndpoints, resource.coverPaths, resource.path, reloadKey]);
