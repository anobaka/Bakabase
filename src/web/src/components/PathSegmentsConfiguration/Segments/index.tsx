...
const Segments = ({
  ...
  const buildSelectiveMatcherRightContent = (
    ...
    const firstLineNodes: React.ReactNode[] = [
      buildMatchModeTip(
        t<string>("Layer"),
        <SisternodeOutlined className={"text-sm"} />,
        l.available,
        l.errors,
      ),
      buildMatchModeTip(
        t<string>("Regex"),
        <FieldBinaryOutlined className={"text-sm"} />,
        r.available,
        r.errors,
      ),
    ];
  ...
  const renderSegments = (): any => {
    if (segments.length > 0) {
      ...
      const rootPathSegmentIndex = segments.findIndex((s) =>
        s.matchResults.some((b) => b.property.isRootPath),
      );
      const resourceSegmentIndex = segments.findIndex((s) =>
        s.matchResults.some((b) => b.property.isResource),
      );
      ...
      for (let i = 0; i < segments.length; i++) {
        ...
        const sc = (
          <div
            key={i}
            className={`${disabled ? "cursor-default opacity-60" : "cursor-pointer"} px-2 py-1 hover:bg-[var(--bakaui-default)] rounded`}
          >
            {matchResults.length > 0 && (
              <div className={"flex items-start gap-1 flex-wrap"}>
                {matchResults.map((mr) => {
                  ...
                  if (
                    !mr.property.isCustom &&
                    (mr.property.id == ResourceProperty.Resource ||
                      mr.property.id == ResourceProperty.RootPath)
                  ) {
                    colorKey = "success";
                  }
                  if (hasError) {
            ...
            {rootDomRef.current && (
              <Popover
                ...
                trigger={<div className="text-lg">{text}</div>}
              >
                <div className="p-4">
                  <div className={"mb-2"}>
                    <div className={"font-bold text-xl"}>
                      {t<string>("Mark this path segment as")}
                    </div>
                  </div>
                  <div>
                    <Listbox
                      disabledKeys={selectiveMatchers
                        .filter((m) => !m.isConfigurable)
                        .map((m) => selectiveMatchers.indexOf(m))
                        .map((x) => x.toString())}
                      variant={"bordered"}
                      onAction={(k) => {
                  ...
                  <div className={"italic opacity-60 mt-1"}>
                    {t<string>(
                      "Some properties may not able to set by layer, but you still can set it by regex.",
                    )}
                  </div>
                </div>
              </Popover>
