...
class PscMatcher {
  ...
  isRequired: boolean;
  order: number;
  prerequisites: PscPropertyType[] = [
    PscPropertyType.Resource,
    PscPropertyType.RootPath,
  ];
  checkOrder: number;
  ...
  static match(
    segments: string[],
    value: IPscMatcherValue | undefined,
    startIndex: number | undefined,
    endIndex: number | undefined,
  ): PscMatchResult | undefined {
      ...
      switch (value.valueType) {
            ...
            if (value.layer > 0) {
              if (startIndex != undefined && startIndex >= -1) {
                ...
                if (idx < segments.length) {
                  result = PscMatchResult.OfLayer(
                    value.layer,
                    value.layer + startIndex,
                  );
                }
              }
            } else {
              if (
                ...
                endIndex + value.layer >= 0
              ) {
                result = PscMatchResult.OfLayer(
                  value.layer,
                  value.layer + endIndex,
                );
              }
            }
          }
        ...
        case ResourceMatcherValueType.Regex: {
          if (
            ...
            const subPath = segments.slice(startIndex + 1, endIndex).join("/");
