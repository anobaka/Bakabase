      ...
      ? categories
          .map((a) => {
            if (a.name.toLowerCase().indexOf(lowerCasedKeyword) > -1) {
              ...
              const filteredLibraries = (a.libraries || [])
                .map((b) => {
                  if (b.name.toLowerCase().indexOf(lowerCasedKeyword) > -1) {
                    return b;
                  } else {
                    const filteredPaths = b.paths.filter((c) => {
                      return c.toLowerCase().indexOf(lowerCasedKeyword) > -1;
                    });
