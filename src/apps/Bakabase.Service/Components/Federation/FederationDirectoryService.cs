...
public sealed class FederationDirectoryOpener : IFederationDirectoryOpener
{
    ...
    public void Open(string directory, CancellationToken ct = default)
    {
        ...
        else if (OperatingSystem.IsMacOS())
        {
            ...
            // LaunchServices runs an application bundle rather than showing it, and a mapped share's
            // names are chosen by its owner. Packages are only revealed in Finder, never opened.
            if (IsMacPackage(directory)) start.ArgumentList.Add("-R");
        }
    ...
    {
        ".app", ".appex", ".bundle", ".component", ".framework", ".kext", ".mdimporter", ".mpkg", ".osax",
        ".pkg", ".plugin", ".prefpane", ".qlgenerator", ".saver", ".scptd", ".service", ".systemextension",
        ".vst", ".vst3", ".wdgt", ".workflow", ".xpc"
    };

    internal static bool IsMacPackage(string directory) =>
        MacPackageExtensions.Contains(Path.GetExtension(Path.TrimEndingDirectorySeparator(directory))) ||
        File.Exists(Path.Combine(directory, "Contents", "Info.plist"));
}
