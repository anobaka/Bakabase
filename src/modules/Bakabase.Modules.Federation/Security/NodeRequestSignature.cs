...
public static class NodeRequestSignature
{
    ...
    /// <summary>
    /// Covers the identity and clock a peer relies on, as a fixed field list rather than serialized
    /// <see cref="NodeInfo"/>, so later versions can add fields without breaking older peers' proofs.
    /// </summary>
    public static string HandshakeProof(string key, NodeInfo info, string challenge) =>
        Mac(key, string.Join("\n", "Bakabase-Node-handshake-v1", challenge, info.NodeId, info.LibraryEpoch, info.Name,
            info.ProtocolVersion.ToString(CultureInfo.InvariantCulture),
            info.ServerTimeUtc.ToUniversalTime().ToString("O", CultureInfo.InvariantCulture)));
