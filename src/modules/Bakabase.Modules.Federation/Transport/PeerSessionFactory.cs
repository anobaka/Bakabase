...
public sealed class PeerSessionFactory(FederationStateStore store, INodeIdentityProvider identity,
    FederationHttpClient http, TimeProvider timeProvider) : IPeerSessionFactory
{
    ...
    public async Task<PeerSessionSnapshot> GetAsync(string nodeId, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var info = await http.PublicAsync<NodeInfo>(peer.Address, HttpMethod.Get, "/federation/v1/info", null,
                cancellationToken);
            ...
            using var request = FederationHttpClient.CreateRequest(peer.Address, HttpMethod.Post,
            ...
            var snapshot = new PeerSessionSnapshot(nodeId, peer.Address, credentials, verifiedOffset, proof.Info,
