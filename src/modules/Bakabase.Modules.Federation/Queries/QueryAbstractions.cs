...
public sealed record FederationQueryLimits
{
    ...
    // The only coordinator owner is the local UI; every window and a search replacing its
    // predecessor (whose release may still be in flight) share it. Budgets bound the memory.
    public int MaxSessionsPerOwner { get; init; } = 8;
