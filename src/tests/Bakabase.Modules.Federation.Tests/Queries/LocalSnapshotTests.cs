...
public class LocalSnapshotTests
{
    ...
    [TestMethod]
    public void ProjectionCarriesOnlyProtocolSourceKinds()
    {
        // Removed sources (Bangumi=6, VNDB=8) linger in older libraries; a single one on the
        // wire would make every coordinator reject the whole block.
        var capture = new LocalLibraryCapture("node", "epoch", "Node", DateTimeOffset.UtcNow, DateTimeOffset.UtcNow, []);
        var summary = QueryProtocol.Project(capture, new(1, "Title", null, true, [8, 7, 6, 1, 7]));
        CollectionAssert.AreEqual(new[] { 1, 7 }, summary.SourceKinds);
    }
