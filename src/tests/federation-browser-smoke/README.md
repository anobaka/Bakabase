...
6. First launch (`first-launch.cjs`): a desktop install started for the first time after that — its own data directory, the thin client's to read — has brought the thin client's pairing with the source over by itself when it reports ready, before any page is opened: the listing shows it imported, with its path mappings and no key; the store holds the thin client's device ID and key, mode `0600`, and records that the import ran; the thin client's file is unchanged.
