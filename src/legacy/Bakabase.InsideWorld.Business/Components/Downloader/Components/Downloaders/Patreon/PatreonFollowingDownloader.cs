...
{
    public class PatreonFollowingDownloader : AbstractPatreonDownloader
    {
        public override PatreonDownloadTaskType EnumTaskType => PatreonDownloadTaskType.Following;

        public PatreonFollowingDownloader(IServiceProvider serviceProvider, ISpecialTextService specialTextService) : base(serviceProvider, specialTextService)
        {
        }
        ...
        {
            throw new NotImplementedException("Creator download not supported in Following downloader");
        }
        ...
        protected override async Task DownloadFromPledging(DownloadTaskDbModel task, string downloadPath, string namingConvention, CancellationToken ct)
        {
            ...
            Logger.LogInformation("Starting Patreon following download task: {TaskId}", task.Id);
            ...
            try
            {
                ...
                var namingContext = new Dictionary<PatreonNamingFields, object?>
                {
                    ...
                    [PatreonNamingFields.PostTitle] = "Sample Following Post",
                ...
                Logger.LogInformation("Patreon following download task completed: {TaskId}", task.Id);
            }
            catch (Exception ex)
            {
                Logger.LogError(ex, "Error in Patreon following downloader for task: {TaskId}", task.Id);
                throw;
            }
        }
        ...
        {
            throw new NotImplementedException("Tier download not supported in Following downloader");
        }
        ...
        {
            throw new NotImplementedException("Campaign feed download not supported in Following downloader");
        }
    }
}