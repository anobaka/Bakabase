...
{
    public class PatreonCreatorDownloader : AbstractPatreonDownloader
    {
        public override PatreonDownloadTaskType EnumTaskType => PatreonDownloadTaskType.Creator;

        public PatreonCreatorDownloader(IServiceProvider serviceProvider, ISpecialTextService specialTextService) : base(serviceProvider, specialTextService)
        {
        }
        ...
        protected override async Task DownloadFromCreator(DownloadTask task, string downloadPath, string namingConvention, CancellationToken ct)
        {
            ...
            Logger.LogInformation("Starting Patreon creator download task: {TaskId}", task.Id);
            ...
            try
            {
                ...
                var namingContext = new Dictionary<PatreonNamingFields, object?>
                {
                    ...
                    [PatreonNamingFields.PostTitle] = "Sample Creator Post",
                ...
                Logger.LogInformation("Patreon creator download task completed: {TaskId}", task.Id);
            }
            catch (Exception ex)
            {
                Logger.LogError(ex, "Error in Patreon creator downloader for task: {TaskId}", task.Id);
                throw;
            }
        }
