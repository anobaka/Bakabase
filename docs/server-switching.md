...
一体版是所有 PC 的唯一桌面产品：它始终运行本机服务端，同时可以把主窗口切换到已配对的其他服务端（NAS，或另一台 PC 上的一体版），并像原纯客户端一样完全控制它。无界面的 NAS/Docker 镜像只会被管理，不会管理其他设备。纯客户端停止维护，其配对由一体版直接导入。
...
6. **从纯客户端迁移**：一体版首次启动时自动读取本机纯客户端的配对（含密钥与路径映射）并导入，已存在的设备不覆盖；设备页可再次手动导入。纯客户端的连接页和界面会提示改用一体版。
...
- 中继不向对方页面暴露本机诊断信息：纯客户端的 `/client/log*`、`/client/app/*`（日志、应用目录路径、打开目录）只在纯客户端中提供；一体版中这些是本机整个应用的日志（含本机服务端打印的配对码、所有受管设备的地址）和保存所有密钥的数据目录，中继一律返回 404。
...
| 位置 | 内容 |
| --- | --- |
| `src/apps/Bakabase.Remoting/Components/Relay` | 中继的公共组成（纯客户端与一体版共用） |
