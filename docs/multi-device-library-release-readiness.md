...
代码和自动化已提供可执行的发布检查。较早 `d77021ed` 修复 Intel 原生 WebView 崩溃，并通过六包安装、三平台双产品并存、原生更新/自动重启与数据保留（含 Mac 实际系统授权）。本轮另修复 mpv 网络后端兼容和查询准备调度，当前产品 `ed61e4cc` 的六种安装包、四平台大库统计，以及 Windows 真实已发布旧包升级至 core400 均通过。按 macOS 测试版优先保留数据的要求，ARM 原 v349 程序建库、原生 updater 自动更新至 core400、再次重启后的七表原字段、配置与媒体保留已通过。该测试显式激活未修改的旧可执行文件，不证明原旧 bundle 的正常系统入口可用；Intel 旧客户端另有 SIGILL 启动限制。原生 GUI、播放器及完整回归的精确来源和结果见扩展验收记录；Intel 完整原生界面、生产签名/公证、物理设备网络和生产通道继续保留独立门禁。
...
| 更新与数据隔离 | stable/beta 各按原 feed 更新，重启后有效 AppData 不变，独立单实例/端口/进程并存，原库 SQLite 完整性可复核 | `d77021ed` 三平台原生更新与 Mac 授权通过；本轮 Windows 原 v349→core400 默认缓存/自动重启/数据保留通过。ARM 原 v349 建库后的原生更新/重启、Intel 新版恢复真实 ARM 旧数据后的自身原生更新/重启均通过七表字段、配置、媒体保留。Intel 使用同产品字节重打包更新，不证明旧 Intel updater；原旧 bundle 正常入口与 Intel 旧客户端 SIGILL 单独保留，生产 stable/beta 通道未关闭 |
| 完整 GUI 主路径 | 最终桌面壳首启、空库设置/日志、开启浏览、配对、联合查询、详情、播放、离线、恢复、关闭窗口；确认远端不启动播放器 | 当前候选已有 Windows/ARM 原生空库/开启浏览/范围搜索/状态保留证据，最新 Windows 再次通过。Mac direct AX 的 CF 引用、typed 参数和 CFIndex 格式问题已验证修复，但读取 11 个节点后遇到未获证实的跨进程归属，0 动作，不能放行；三平台完整配对/详情/离线恢复主流程未完成，较早 ARM 手工原生证据另记 |
