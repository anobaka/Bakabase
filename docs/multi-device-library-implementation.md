...
最新的原生播放器、跨平台性能、界面与已发布旧包升级结果见[扩展验收记录](multi-device-library-acceptance-20260922.md)。本文较早的通过数量保留原提交归属，不能覆盖扩展验收发现的失败或替代修改后的复验。

当前产品 `ed61e4cc` 已包含 mpv 网络后端兼容与查询准备调度修复。完整 CI 的前端 936 项、后端 2,586 项通过（另有 35 项既有显式跳过），四平台各 311 项专项测试及六种候选安装包通过；四平台双库 10k/100k 共 48 次遍历和 8 次取消/额度复用通过。Windows 真实已发布 v349 两产品升级至 core400 通过；Mac 原 v349 首次正常启动失败、Intel 原生 GUI 读取未完成及完整发布矩阵保留在验收结论中。
