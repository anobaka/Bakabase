...
> ## ⚠️ 重要：2.3-beta 用户请阅读
>
> **不要**在任何 `2.3-beta.*` 版本中使用应用内升级 —— 会导致 AppData 被清空。请立即将数据文件夹（路径见 **设置 → 系统信息 → 应用数据路径**）备份到 Bakabase 安装目录之外的位置。版本 `< 2.3` 的用户暂勿升级。详情与修复进度见 [#1070](https://github.com/anobaka/Bakabase/issues/1070)。macOS 不受影响。
