...
> ## ⚠️ 从 2.3 之前的版本升级
>
> 2.3 采用了全新的安装器与自动更新机制，**无法从 2.3 之前的版本就地升级**。请手动下载一次 2.3 安装包并全新安装；此后每次更新都可在应用内一键完成。若想保留旧版本的数据，先启动新装好的 2.3，再到 **设置 → 系统信息 → 应用数据路径 → 修改** 指向你的旧数据目录即可。
